	...
	if (isClass(configFile >> "CfgPatches" >> "ace_medical_engine")) then {
		["ace_unconscious", {
			...
			//	if (primaryweapon _unit != "" && _willdrop) then {_unit remoteexecCall ["ace_common_fnc_throwWeapon", _unit]};
				...
				[{(_this select 0) == vehicle (_this select 0)}, {[(_this select 0), (_this select 1)] remoteexecCall ["zsn_fnc_surrendercycle", (_this select 0)]}, [_unit, _ms]] call CBA_fnc_waitUntilAndExecute;
			//	[_unit, _ms] spawn {
			//		params ["_unit", "_ms"];
